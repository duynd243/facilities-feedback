 ...
 * @author Duy
 */
@WebServlet(name = "AssignEmployeeController", urlPatterns = {"/assign-employee"})
public class AssignEmployeeController extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String assignTime = TimeUtils.currentTimeString();
        String feedbackID = request.getParameter("feedbackID");
        String handlerEmail = request.getParameter("handlerEmail");

        FeedbackDAO dao = new FeedbackDAO();
        try {
            dao.assignEmployee(feedbackID, handlerEmail, assignTime);
        } catch (Exception e) {
        }
